...
def get_result_dict_madpose(stats, pose_est, R_gt, t_gt, f1_gt, f2_gt):
    ...
    out['f1_gt'] = f1_gt
    out['f1'] = pose_est.focal
    out['f2_gt'] = f2_gt
    out['f2'] = pose_est.focal
...
def eval_experiment(x):
    ...
    max_dim_1 = max(2 * K1[0, 2], 2 * K1[1, 2])
    max_dim_2 = max(2 * K2[0, 2], 2 * K2[1, 2])
    max_dim = max(max_dim_1, max_dim_2)
    min_dim = min(max_dim_1, max_dim_2)
    ...
    if '6p' in experiment:
        ...
        start = perf_counter()
        pose, info = madpose.HybridEstimatePoseScaleOffsetSharedFocal(kp1, kp2, d[:, 0], d[:, 1],
                                                                      [d[:, 0].min(), d[:, 1].min()],
                ...
                for experiment in experiments:
                    for iterations in iterations_list:
                        ...
                        if (K1[0, 0] + K1[1, 1]) != (K2[0, 0] + K2[1, 1]):
                            ...
                            K2 = (K1[0, 0] + K1[1, 1]) / (K2[0, 0] + K2[1, 1]) * K2
